...
flowchart TB
    ...
    classDef prompt fill:#dcfce7,stroke:#16a34a,color:#111
    classDef rag fill:#dbeafe,stroke:#2563eb,color:#111
    classDef ft fill:#fee2e2,stroke:#dc2626,color:#111
...
Claude'un [Model Spec](https://docs.claude.com/en/docs/model-spec)'i tanımlı davranış listesi içerir (zararlı isteği reddetme, dürüstlük, kullanıcı güvenliği). FT bu tanımı bozabilir. Müşteri "benim için FT edin" derse, Anthropic şöyle sorar: "Hangi Model Spec maddesi sana çelişki yaratıyor? Belki yanlış yoldasın."
...
- **A → B:** AI Engineer'ın model öğretme araç kutusu 3 parça: prompt + RAG + FT; her biri farklı problem için.
- **B → C:** Fine-tune 3 biçim: tam FT (enterprise-only), LoRA (adapter, tek GPU), QLoRA (4-bit + tüketici GPU).
- **C → D:** Instruction tuning %95 geliştiricinin yaptığı FT; chat format modern standart.
- **D → E:** Veri 200-2000 örnek optimum; miktar yerine kalite + çeşitlilik.
- **E → F:** QLoRA $0-10 (Colab) vs tam FT $5K-50K — 1000× fark.
- **F → G:** Araçlar: HF Transformers + PEFT + TRL + Unsloth + Axolotl ekosistemi.
- **G → H:** Managed: OpenAI/Vertex/Bedrock var; Anthropic public FT sınırlı — Constitutional AI tutarlılığı için.
- **H → I:** "FT gerektirmez" %90 iddiası 7 nedenle: maliyet, zaman, bakım, version, infra, geri alınamaz, Claude Model Spec.
