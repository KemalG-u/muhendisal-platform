...
!!! tip "Yabancı kelime mi gördün?"
    Bu sayfadaki **italik-altı çizili** ifadelerin (quantization, inference, GGUF gibi) üstüne mouse'unu getir — kısa tanım çıkar.
...
**Quantization = modeli ufaltma tekniği.** Orijinal Llama 3.2 (8B) 16 GB disk + 16 GB RAM ister. **Quantize edilmiş** (4-bit) sürümü ~5 GB'a düşer, 8 GB RAM'de çalışır. Kalite %3-5 düşer, hız %40 artar. Ollama default 4-bit quantize modelleri indirir — pratik için ideal. Production kalitesi isterseniz 8-bit veya full precision.
...
flowchart TB
  ...
  classDef yerel fill:#fef3c7,stroke:#ca8a04,color:#111
  classDef bulut fill:#fed7aa,stroke:#ea580c,color:#111
  classDef out fill:#dcfce7,stroke:#16a34a,color:#111
  class USR usr
  class PROMPT veri
  class FORK karar
  class OLLAMA,MODEL yerel
  class CLAUDE bulut
  class LOCAL_OUT,CLOUD_OUT out
```
...
??? info "Teknik detay — isteyene (parameter adları, mekanikler, edge case'ler)"
    ...
    **Güvenlik dikkati.** Ollama default herkes erişebilir (`localhost:11434`). VPS'te kurarsan firewall kapat (`ufw deny 11434`) veya reverse proxy + auth koy. Prompt injection Ollama modellerinde **daha kolay** — Claude Constitutional AI yok.
...
- **A → B:** Her AI çağrısı Claude'a gitmek zorunda değil — development + gizlilik + offline için yerel LLM çözüm.
- **B → C:** Ollama 2023'ten beri yerel LLM'in Docker'ı haline geldi — tek komut, Mac/Linux/Windows, OpenAI-uyumlu API.
- **C → D:** Quantization sayesinde 8 GB RAM'li laptop'ta bile 3B model çalışıyor — donanım engeli ortadan kalktı.
- **D → E:** Türkçe kalitesinde Qwen > Llama > Phi; multimodal ve uzun bağlamda Claude hep önde.
- **E → F:** Hybrid desen (env değişkeni ile yerel/bulut switch) development hızını artırır, production kalitesini korur.
