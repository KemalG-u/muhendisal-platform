...
<div class="ma-meta-row"><strong>🎯 Çıktı:</strong> 4 vision model karşılaştırma refleksin elinde — **Claude Sonnet 4.5** vs **GPT-4o** vs **Gemini 2.5 Pro** vs **Qwen2-VL** (açık kaynak). 5 benchmark (OCR, Türkçe, diyagram, chart, sahne) üzerinde model seçim matrisi. **Mülakatta "hangi vision model?"** sorusuna **gerekçeli** cevap. **Bölüm 7 kavramsal İMZA** — 9.6 pratik imza için zemin.</div>
</div>

!!! tip "Yabancı kelime mi gördün?"
    **Vision-Language Model (VLM)** = görsel + metin birlikte işleyen model; Claude Sonnet 4.5, GPT-4o, Gemini 2.5 hepsi VLM. **Benchmark** = standart test seti; MMMU, DocVQA, MathVista akademik, aynı veri farklı modeller. **OCR benchmark** = görselden metin okuma doğruluk; DocVQA + InfoVQA. **Multimodal reasoning** = görsel + düşünce zinciri birlikte; "bu grafiği incele, en yüksek değerin sebebi ne?" tipi sorular. **Open weights** = model ağırlıkları açık; Qwen2-VL, LLaVA, Pixtral (Mistral) örnek.
...
|---|---|---|---|
| **Türkçe belge OCR** | Claude Sonnet 4.5 | DocVQA + Türkçe üstün | Gemini 2.5 |
| **UI mockup → kod** | Claude Sonnet 4.5 | Anthropic Claude Code ekosistemi | GPT-4o |
...
| **Genel günlük kullanım** | Claude Sonnet 4.5 | Consistent kalite + Türkçe | GPT-4o |
| **Computer use / agent** | Claude Sonnet 4.5 | Resmi destek + Model Spec | - |
...
**Senin için (genel AI Engineer):** **Claude Sonnet 4.5** default. Gerektiğinde diğer modeller.
...
**Enterprise + Anthropic customer:** Claude Sonnet 4.5 default, tüm akış tek vendor.
...
**Beklenen sonuç:**
- **Claude Sonnet 4.5** Türkçe + yapılandırılmış çıktı + tool calling = **Seçim**
...
|---|---|---|---|
| **Claude Sonnet 4.5** | $4.50 | $7.50 | **$12/ay** |
...
Claude Sonnet 4.5 **computer use** (Ekim 2024) ile ekran görür + fare/klavye simüle eder. Diğer 3 model bunu **yapmaz** (2026 Nisan).
...
|---|---|---|
| **Kapalı commercial** | Claude Sonnet 4.5, GPT-4o, Gemini 2.5 | API odaklı |
...
- **A → B:** 4 ana VLM 2026 Nisan: Claude Sonnet 4.5, GPT-4o, Gemini 2.5 Pro, Qwen2-VL.
